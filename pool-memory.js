// pool-memory.js — Per-pair performance memory for RRL-Futures
// Tracks win rates, performance patterns, scores, and cooldowns per trading pair
...
export function saveMemory() {
  try {
    writeFileSync(MEMORY_FILE, JSON.stringify(_memory, null, 2));
  } catch (e) {
    logger.error(MOD, `Could not save memory: ${e.message}`);
  }
}
...
export function getPairMemory(symbol) {
  return initPair(symbol);
}
...
export function recordPairTrade(symbol, trade) {
  ...
  mem.winRate = mem.totalTrades > 0 ? (mem.wins / mem.totalTrades) * 100 : 0;
  mem.lastTrade = new Date().toISOString();

  if (trade.holdMinutes) {
    const prev = mem.avgHoldMinutes * (mem.totalTrades - 1);
    mem.avgHoldMinutes = (prev + trade.holdMinutes) / mem.totalTrades;
  }

  if (trade.regime) {
    mem.regimeHistory.push({
      regime: trade.regime,
  ...
  // Recalculate score based on recent performance
  const recent = mem.regimeHistory.slice(-10);
  const recentWins = recent.filter(r => r.outcome === 'win').length;
  const recentWR = recent.length > 0 ? recentWins / recent.length : 0.5;
  mem.score = Math.min(100, Math.max(0,
    50
    + (recentWR - 0.5) * 60
    + Math.min(20, Math.max(-20, mem.avgPnl * 10))
  ));

  // Cooldown: 3 losses in a row → 2h blackout
  ...
  if (last3.length === 3 && last3.every(r => r.outcome === 'loss')) {
    ...
    logger.warn(MOD, `${symbol} on 2h cooldown after 3 consecutive losses`);
  }

  saveMemory();
  logger.info(MOD, `${symbol} memory updated — score: ${mem.score.toFixed(1)}, winRate: ${mem.winRate.toFixed(1)}%`);
}

export function isPairOnCooldown(symbol) {
  const mem = _memory[symbol];
  if (!mem || !mem.skipUntil) return false;
  return new Date(mem.skipUntil) > new Date();
}
...
export function getMemoryContext(symbol) {
  const mem = _memory[symbol];
  if (!mem || mem.totalTrades === 0) return `No historical memory for ${symbol} yet.`;
  return `${symbol}: ${mem.totalTrades} trades, winRate: ${mem.winRate.toFixed(1)}%, avgPnl: ${mem.avgPnl.toFixed(2)}, score: ${mem.score.toFixed(1)}/100`;
}
